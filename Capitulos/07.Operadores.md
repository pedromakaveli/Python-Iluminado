...
É um tanto quanto importante ressaltar a importância da ordem de precedência dos operadores aritméticos a ordem de precedência dos operadores aritméticos será de grande ajuda quando necessitarmos de fazer calculos matemáticos.
Quando fazemos a seguinte expressão: 5 + 3 * 3 o Python irá retornar o valor 14. Agora pense um pouco, alguém pode ler da seguinte forma a nossa expressão: 5 + 3 vai ser igual a 8 e 8 * 3 irá ser 24. O que seria um valor errado para a resposta! Pelo motivo de fugir da ordem de precedência. A ordem de precedência irá indicar qual operador deve ser executado em primeiro lugar, então logo a baixo você pode visualizar a ordem de precedência dos operadores:
